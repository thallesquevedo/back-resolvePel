  ...
  beforeEach(async () => {
    ...
    describe('findPrestadorOrdemServicoById', () => {
        it('should return the service order for a user', async () => {
            ...
            reqServicoRepository.createQueryBuilder().getOne.mockResolvedValue(ordemServico);
      
            expect(await service.findPrestadorOrdemServicoById(ordemServicoId, user)).toBe(ordemServico);
        ...
        it('should throw BadRequestException if user does not match', async () => {
            ...
            await expect(service.findPrestadorOrdemServicoById(ordemServicoId, user)).rejects.toThrow(BadRequestException);
        });
    });
    ...
    describe('createReqServico', () => {
        it('should create a new service request', async () => {
            const user = { id: '1' } as User;
            const createReqServicoDto = { servicoId: 1, itemIds: [1, 2], descricao: 'desc' };
            ...
            expect(await service.createReqServico(user, createReqServicoDto)).toBe(reqServico);
        ...
        it('should throw BadRequestException if data is missing', async () => {
            const user = { id: '1' } as User;
            const createReqServicoDto = { servicoId: null, itemIds: [1, 2], descricao: 'desc' };
      
            await expect(service.createReqServico(user, createReqServicoDto)).rejects.toThrow(BadRequestException);
        });
    ...
    describe('updateReqServico', () => {
        it('should update an existing service order', async () => {
            ...
            const user = { id: '1' } as User;
            const updateReqServicoDto = { servicoId: 1, itemIds: [1, 2], descricao: 'new desc' };
            const ordemServico = { id: reqServicoId, user, ...updateReqServicoDto, updated_at: new Date() } as any;
            ...
            const updatedOrdemServico = await service.updateOrdemServico(reqServicoId, user, updateReqServicoDto);
            ...
            expect(servicoService.findOne).toHaveBeenCalledWith(updateReqServicoDto.servicoId);
            expect(itemService.findAllByIds).toHaveBeenCalledWith(updateReqServicoDto.itemIds);
        ...
        it('should throw BadRequestException if user does not match', async () => {
            const reqServicoId = '1';
            const user = { id: '1' } as User;
            const updateReqServicoDto = { descricao: 'new desc', itemIds: [1, 2], servicoId: 1 };
            const ordemServico = { id: reqServicoId, user: { id: 2 } } as any;
            reqServicoRepository.findOne.mockResolvedValue(ordemServico);
      
            await expect(service.updateOrdemServico(reqServicoId, user, updateReqServicoDto)).rejects.toThrow(BadRequestException);
        });
          ...
          it('should throw InternalServerErrorException on error', async () => {
            ...
            await expect(service.updateOrdemServico(reqServicoId, user, updateReqServicoDto)).rejects.toThrow(InternalServerErrorException);
        });
    });
    ...
    describe('deleteOrdemServico', () => {
        ...
        it('should throw BadRequestException if user does not match', async () => {
            ...
            await expect(service.deleteOrdemServico(ordemServicoId, user)).rejects.toThrow(BadRequestException);
        });
    });
    ...
    describe('findAllByCliente', () => {
        it('should return all services by client', async () => {
          const result = [{ id: 1, descricao: 'test' }];
          reqServicoRepository.createQueryBuilder().getMany.mockResolvedValue(result);
    ...
    describe('findClientOrdemSevicoById', () => {
        it('should return a specific service order by client', async () => {
          ...
          reqServicoRepository.createQueryBuilder().getOne.mockResolvedValue(result);
    
          expect(await service.findClientOrdemSevicoById(ordemServicoId)).toBe(result);
          expect(reqServicoRepository.createQueryBuilder).toHaveBeenCalled();
        });
    });
